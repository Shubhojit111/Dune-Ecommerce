...
export default function ProductQuickView({ product }) {
  ...
  const productSecondImage =
    product?.secondImage || product?.hoverImage || productImage;
  ...
  const [specOpen2, setSpecOpen2] = useState(false);
  ...
  return (
    <section className="w-full pt-8 pb-6 lg:pb-10">
      <div className="px-4 sm:px-8 lg:px-10 w-full">
        <div className="flex flex-col md:flex-row gap-10 lg:gap-12 items-start w-full min-w-0">
          {/* LEFT: Product image — thumbnails column + smaller main image */}
          <div className="w-full md:w-[52%] min-w-0 flex-shrink-0 md:sticky md:top-6">
            <div className="flex flex-row gap-3">
              {/* Thumbnail column */}
              <div className="flex flex-col gap-3 overflow-y-auto max-h-[420px] lg:max-h-[500px] [&::-webkit-scrollbar]:hidden flex-shrink-0">
                {THUMBNAILS.map((src, i) => (
                  <button
                    key={i}
                    onClick={() => setMainImage(src)}
                    className={`relative flex-shrink-0 w-[56px] h-[56px] sm:w-[64px] sm:h-[64px] overflow-hidden bg-sand transition-all duration-200 border-2 ${
                      mainImage === src
                        ? "border-ink"
              ...
              {/* Main image */}
              <div className="relative flex-1 min-w-0 h-[380px] sm:h-[420px] lg:h-[500px] bg-sand overflow-hidden">
                <Image
                  src={mainImage}
                  alt="Camping Hoodie"
                  fill
          ...
          <div className="w-full md:w-[48%] min-w-0 flex-1 md:py-2 flex flex-col items-center sm:items-start">
            {/* Breadcrumb */}
            ...
            {/* Monsoon Special Sale Offer banner */}
            ...
            <div className="flex flex-col gap-3 mb-8 w-full">
              {cartError && (
                <p className="text-[12px] text-red-600 text-center mb-1">
                  {cartError}
                </p>
              )}
              <AddToCart
