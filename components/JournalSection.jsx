          ...
          {CARDS.map((card) => (
            <article key={card.title} className="flex flex-col">
              ...
              <div className="flex flex-col items-center text-center pt-8 sm:pt-9 pb-2">
                ...
                <SubTextBtn text={card.copy} className="text-center !text-ink/80 " />
                <Link
                  href={card.href}
                  className="inline-flex items-center justify-center h-[44px] px-7 border border-ink/40 rounded-full text-[11.5px] uppercase tracking-[0.25em] font-bold text-ink hover:border-ink hover:bg-ink hover:text-cream transition-all duration-300"
                >
                  {card.cta}
                </Link>
              </div>
